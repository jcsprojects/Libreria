...
class GalleryView {
  constructor() {
    ...
    this.containerLogin = $(".login-container");
  }
  ...
  showLogin() {
    this.miLogin.hide();
    let avatar = this.getCookie("usuario");
    this.containerLogin.append(`

  <div class="chip">
    <img src="img/img_avatar.jpg" alt="Person" width="96" height="96">
   ${avatar}
  </div> 
  <div>
    <a href="#" id="close" class="nav-item nav-link active"> Cerrar Sesion</a>
  </div>
  `);
  }
  showImageTypes() {
  ...
  showFavorites() {
    ...
    let favoritas = JSON.parse(localStorage.getItem("miAlmacen"));
    // console.log(favoritas);
    console.log(favoritas[0].title);
    this.main.empty();
    this.categories.empty();
    this.authors.empty();
    this.tipo.empty();
    ...
    for(let i=0;i<favoritas.length;i++) {
      let div = $(`<div class="col-md-4">
				...
				<figure class="card card-image-grid card-lg border"> <a data-title="${favoritas[i].title}" href="#single-image" class="img-wrap"><img class="${favoritas[i].title}-style" src="${favoritas[i].url}"></a>
					<figcaption class="info-wrap ">
						<div class="row ">
							<div class="col-md-8"> <a data-title="${favoritas[i].title}" href="#single-image" class="title">${favoritas[i].title} - ${favoritas[i].description}</a> </div>
							<div class="col-md-4">
								<div class="rating text-right"> <i class="fa fa-star"></i> <i class="fa fa-star"></i> <i class="fa fa-star"></i> </div>
							</div>
						</div>
					</figcaption>
					<div class="bottom-wrap">  <a href="#" class="btn btn-primary float-left"> <i class="bi bi-bookmark-heart-fill"></i>Ver</a></div>
  ...
  showFavoritesInMenu() {
    let li = $(`<a id="favorites" class="nav-link" href="#">Favoritas
    ...
    while (!image.done) {
      let div = $(`<div class="col-md-4">
				<figure class="card card-image-grid card-lg border"> <a data-title="${image.value.title}" href="#single-image" class="img-wrap"><img class="${image.value.title}-style" src="${image.value.url}"></a>
					...
					<div class="bottom-wrap" id="add"> 
					 <a href="#"  data-title="${image.value.title}" class="btn btn-primary float-left"> <i class="bi bi-bookmark-heart-fill"></i>Añadir a favoritos</a></div>
				</figure>
			</div>`);
      container.children().first().append(div);
      image = images.next();
    ...
    if (image) {
      ...
      <div class="row justify-content-center h-100 ">
            ...
            <div class="card-body border">
              ...
              <a href="#" id="${image.value.title}" class="btn btn-primary float-left"> <i class="bi bi-bookmark-heart-fill"></i>Añadir a favoritos</a>
            </div>
          </div>
          </div>
			</div>
  ...
  showRemoveAuthorForm(authors) {
    ...
    let author = authors.next();
    while (!author.done) {
      container.children().nextAll("div").append(
        `<div class="aut col-lg-3 col-md-6"><a data-author="${author.value.nickname}" href="#image-list"> 
            <div class="cat-list-image">
              <img alt="${author.value.nickname}" src="img/${author.value.nickname}.jpg" /> 
              </div> 
            <div class="cat-list-text"> 
              <h3>${author.value.nickname}</h3> 
            <div>
            <button class="btn btn-primary" data-author="${author.value.nickname}" type='button'>Eliminar</button></div> 
            </div> 
            </a> 
            </div>`
      );
  ...
  showRemoveImageModal(done, image, position, error) {
    ...
    if (done) {
      ...
      removeCategoryModal.find("button").click(() => {
        ...
        removeCategoryModal.modal("hide");
        let divCat = $("#image-list").find(
          `div > div:nth-child(${position + 1})`
        );
        divCat.remove();
      });
    } else {
      $("#remove-image").prepend(
        `<div class="error text-danger p-3"><i class="fas fa-exclamation-triangle"></i> La imagen <strong>${image.title}</strong> no exite en la galeria.</div>`
      );
    }
  }
